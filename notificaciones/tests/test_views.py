...
class UsuariosViewsExpandedTest(TestCase):
    ...
    def test_registro_view_duplicate_email_error(self):
        ...
        self.assertIn('Este correo ya está registrado', str(form.errors['email']))
    ...
    def test_vistas_roles_context_variables(self):
        ...
        for vista in vistas:
            with self.subTest(vista=vista):
                ...
                self.assertIn('is_organizador', response.context)
                self.assertIn('is_revisor', response.context)
                self.assertIn('is_administrador', response.context)

                # Para usuario multi-rol, debería ser True para organizador y revisor
                self.assertTrue(response.context['is_organizador'])
                self.assertTrue(response.context['is_revisor'])
    ...
    def test_actualizar_roles_remove_role(self):
        ...
        # Remover rol de Organizador (no enviando el checkbox)
        response = self.client.post(self.urls['actualizar_roles'], {
            f'roles_{usuario_test.id}_revisor': 'on',  # Mantener revisor
            f'roles_{usuario_test.id}_administrador': 'on'  # Mantener admin
        ...
        # Verificar que se creó la notificación de remoción
        new_notifications = Notificacion.objects.filter(usuario=usuario_test).count()
        self.assertEqual(new_notifications, initial_notifications + 1)
...
class CustomUserModelTest(TestCase):
    ...
    def test_email_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            ...
            CustomUser.objects.create_user(**user_data_2)

    def test_username_unique_constraint(self):
        """Test restricción de username único"""
        CustomUser.objects.create_user(**self.user_data)
...
class RegistroFormExpandedTest(TestCase):
    ...
    def test_save_method_duplicate_email_validation(self):
        ...
        # Intentar crear form con email duplicado
        form_data = self.form_data.copy()
        form_data['email'] = 'existing@test.com'
        form = RegistroForm(data=form_data)

        # El formulario debería ser válido en validación inicial
        self.assertTrue(form.is_valid())

        # Pero fallar en save() por la validación adicional
...
class IntegrationTest(TestCase):
    ...
    def test_complete_user_lifecycle(self):
        ...
        # 1. Registro
        registro_response = self.client.post(reverse('registro'), {
            'nombre': 'Integration',
            'apellidos': 'Test',
...
class SpecificLineCoverageTest(TestCase):

    def test_forms_line_48_51_validation(self):
        # Test validación específica de RegistroForm
        form = RegistroForm(data={
            'email': 'test@test.com',
        })

    def test_models_str_method(self):
        user = CustomUser(nombre='Test', apellidos='User', username='test@test.com')
        str_result = str(user)

    def test_models_conferencias_property(self):
        user = CustomUser.objects.create_user(...)
        conferencias = user.conferencias_como_revisor
